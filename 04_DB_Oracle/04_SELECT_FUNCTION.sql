...
SELECT ROUND(AVG(NVL(BONUS, 0)), 2) 평균보너스
FROM EMPLOYEE;

/*
    MIN: 해당 컬럽 값들 중에 가장 작은 값
...
/*
    ...
    - DISTINCT 컬럼: 해당 컬럼값이 중복을 제거한 행 개수 반환
*/
...
SELECT COUNT(BONUS) -- 그룹함수라 NULL 처리 해줌
FROM EMPLOYEE;
-- 부서가 배치된 사원 수 조회
...
-- EMPLOYEE에서 직급별 급여합
SELECT * FROM JOB;
SELECT JOB_NAME 직급, SUM(SALARY) 급여합
FROM EMPLOYEE
...
GROUP BY ROLLUP(JOB_NAME); -- 전체 합계까지 추산
-- CUBE
SELECT JOB_NAME 직급, SUM(SALARY) 급여합
FROM EMPLOYEE
    JOIN JOB USING(JOB_CODE)
GROUP BY CUBE(JOB_NAME); -- 컬럼 하나일 때는 ROLLUP과 차이 X

-- 부서코드와 직급코드가 같은 사원들의 급여 
...
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';
-- 급여가 300만원 초과인 사원들의 사번, 사원명, 부서코드, 급여 조회조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
